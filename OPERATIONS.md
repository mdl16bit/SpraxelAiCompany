...
| **`dashboard.py`** | Always-on TUI dashboard. Auto-refresh every 5 s (configurable via `--interval`). **Multi-project tabs**: global panels (status/tick/token-$/schedule) always cover ALL enabled games; per-game panels show ONE project at a time — **TAB** cycles projects (Shift-TAB back, `1`-`9` jump), tab bar under the title; piped/redirected output stacks every game in full. Compact view: status (incl. `RUNNING/PAUSED (interactive-dev)` from the `/spraxel-develop` heartbeat) / tick / wrapper / cap counter / current item / today's totals + estimated metered $ / **next 10 scheduled fires** / **next 10 CEO action items** (urgency-ordered: `[needs-ceo]` > `[escalated]` > triage questionnaires (`TRIAGE.md`) > play-test > `[bug]` > `[idea]` > MANUAL > dictation backlog; color-coded) / **last 15 shipped** (sha + relative age + clean subject) / last log line. Stdlib only — no tokens, no third-party deps. Run in a terminal you leave open. | CEO, runs continuously while logged in |
