...
def ship(path: Path, title: str) -> WorkItem:
    ...
    completion order. Strips transient process/state tags that are meaningless
    once shipped: [wip:N] (no longer claimed), [retry]/[resume] (no longer
    mid-retry), and [untriaged]/[untriaged-proposal-active] (no longer awaiting
    shaping). The KIND tag ([feature]/[bug]/...) is kept. Raises ValueError if
    not found.
    """
    with FileLock(path):
        wm = parse(path)
        ...
        # Strip transient process/state tags — a shipped item is no longer
        # claimed, retrying, resuming, or awaiting shaping; only its KIND tag
