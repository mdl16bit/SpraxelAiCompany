...
## ~~`run_mode: dryrun` not yet honored in GH Actions workflows~~ Fixed 2026-05-24

Shipped to the 5 LLM-cost workflows: `developer.yml`, `review.yml`, `playtest.yml`, `blogger.yml`, `auto-merge.yml`. Each now has a `dryrun-guard` job that the main job depends on; flipping `Philosophy.run_mode` to `"dryrun"` causes the main job to be skipped.

Not gated (intentional — they don't cost LLM money): `test.yml`, `sync.yml`, `tripwire.yml`, `cost-report.yml`, `inactivity-check.yml`.
